...
def _load_policy_context(log_path):
    ctx = {
        ...
        "under_pick_margin_q80": np.nan,
        "bttsy_pick_prob_q70": np.nan,
        "bttsy_p_over25_q60": np.nan,
        "bttsn_pick_score_q70": np.nan,
    ...
    def _subset(group, pick):
        return log_df[
            (log_df.get("PICK_GROUP") == group)
            & (log_df.get("PICK_FINAL") == pick)
        ].copy()

    ms1 = _subset("1X2", "MS1")
    if not ms1.empty:
        ctx["ms1_p_over25_q60"] = _q(ms1.get("P_OVER25_ADJ"), 0.60)
        ctx["ms1_pick_margin_q70"] = _q(ms1.get("PICK_MARGIN"), 0.70)
    ...
    if not under.empty:
        ctx["under_pick_prob_q90"] = _q(under.get("PICK_PROB_ADJ"), 0.90)
        ctx["under_pick_margin_q80"] = _q(under.get("PICK_MARGIN"), 0.80)

    bttsy = _subset("BTTS", "BTTS_Yes")
    if not bttsy.empty:
        ctx["bttsy_pick_prob_q70"] = _q(bttsy.get("PICK_PROB_ADJ"), 0.70)
        ctx["bttsy_p_over25_q60"] = _q(bttsy.get("P_OVER25_ADJ"), 0.60)

    bttsn = _subset("BTTS", "BTTS_No")
    if not bttsn.empty:
        ctx["bttsn_pick_score_q70"] = _q(bttsn.get("PICK_SCORE_TOTAL"), 0.70)
...
def _policy_flag(row, ctx):
    ...
    if group == "1X2" and pick == "MS1":
        ...
        if np.isfinite(p_over) and np.isfinite(t_over) and (p_over >= t_over):
            if np.isfinite(margin) and np.isfinite(t_margin) and (margin <= t_margin):
                return 1, "MS1_POVER_Q60_MARGIN_Q70"

    # MS2: PICK_SCORE_TOTAL >= q50 & BestOfRank >= q80
    if group == "1X2" and pick == "MS2":
        pick_score = _f(row.get("PICK_SCORE_TOTAL"))
        bor = _f(row.get("BestOfRank"))
        t_score = _f(ctx.get("ms2_pick_score_q50"))
        t_bor = _f(ctx.get("ms2_bestofrank_q80"))
    ...
    # Under 2.5: PICK_PROB_ADJ >= q90 & PICK_MARGIN >= q80
    if group == "OU" and pick == "Under2.5":
        pick_prob = _f(row.get("PICK_PROB_ADJ"))
        margin = _f(row.get("PICK_MARGIN"))
        t_prob = _f(ctx.get("under_pick_prob_q90"))
        t_margin = _f(ctx.get("under_pick_margin_q80"))
        if np.isfinite(pick_prob) and np.isfinite(t_prob) and (pick_prob >= t_prob):
            if np.isfinite(margin) and np.isfinite(t_margin) and (margin >= t_margin):
                return 1, "OU_UNDER_PROB_Q90_MARGIN_Q80"

    # BTTS Yes: PICK_PROB_ADJ >= q70 & P_OVER25_ADJ <= q60
    if group == "BTTS" and pick == "BTTS_Yes":
        pick_prob = _f(row.get("PICK_PROB_ADJ"))
        p_over = _f(row.get("P_OVER25_ADJ"))
        t_prob = _f(ctx.get("bttsy_pick_prob_q70"))
        t_over = _f(ctx.get("bttsy_p_over25_q60"))
        if np.isfinite(pick_prob) and np.isfinite(t_prob) and (pick_prob >= t_prob):
            if np.isfinite(p_over) and np.isfinite(t_over) and (p_over <= t_over):
                return 1, "BTTSY_PROB_Q70_POVER_Q60"

    # BTTS No: PICK_SCORE_TOTAL >= q70 & PICK_MARGIN <= q50
    if group == "BTTS" and pick == "BTTS_No":
        pick_score = _f(row.get("PICK_SCORE_TOTAL"))
        margin = _f(row.get("PICK_MARGIN"))
        t_score = _f(ctx.get("bttsn_pick_score_q70"))
        t_margin = _f(ctx.get("bttsn_pick_margin_q50"))
        if np.isfinite(pick_score) and np.isfinite(t_score) and (pick_score >= t_score):
            if np.isfinite(margin) and np.isfinite(t_margin) and (margin <= t_margin):
                return 1, "BTTSN_SCORE_Q70_MARGIN_Q50"
...
def pick_best_market(row: pd.Series, params=None, debug_context=None):
    ...
    # Prob fallbacks: bazi exportlarda Sim_* kolonlari yok, SIM_IMP_* / P_*_ADJ var
    ...
    for label, _p_col, o_col in markets:
        ...
        bonus = scaled_bonus + elo_factor + open_factor - draw_penalty
...
def render_tabs(pred_df, res, knn, past, future, debug):
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["?? Tahmin Tablosu", "?? Maç Detayi", "?? Sema & Debug", "?? Tahmin Sonuçlari"])
    ...
    if isinstance(res, pd.DataFrame) and not res.empty:
        ...
        with tab1:
            ...
            if isinstance(log_df, pd.DataFrame) and not log_df.empty:
                try:
                    out_dir = os.path.join(os.path.dirname(__file__), "Backtest-Out")
                    ...
                    st.markdown(
                        ...
                        <div class="policy-cards">
                          ...
                          <div class="policy-card">
                            ...
                            <div class="legend">
                              <div class="legend-row"><span class="legend-chip" style="background:#f8d7da;"></span>0–14% · Zayif</div>
                              <div class="legend-row"><span class="legend-chip" style="background:#f3b7b7;"></span>15–28% · Düsük</div>
                              ...
                              <div class="legend-row"><span class="legend-chip" style="background:#e2f0d9;"></span>57–70% · Iyi</div>
                              <div class="legend-row"><span class="legend-chip" style="background:#cbe8c1;"></span>71–84% · Çok Iyi</div>
            ...
            if view.empty:
                ...
                try:
                    matrix_path = os.path.join(os.path.dirname(__file__), "pred_results_log.csv")
                ...
                if isinstance(mtx, pd.DataFrame) and not mtx.empty:
                    ...
                    if required.issubset(mtx.columns):
                        ...
                        if not _m.empty:
                            ...
                            keys = stats["League"].astype(str) + "||" + stats["PICK_FINAL"].astype(str)
                ...
                def _row_bg(r):
                    key = f"{r.get('League','')}||{r.get('PICK_FINAL','')}"
                ...
                if hit_map:
                    view["Hitrate"] = view.apply(
                        lambda r: _bucket_label(hit_map.get(f"{r.get('League','')}||{r.get('PICK_FINAL','')}", np.nan)),
                        axis=1,
                    )
        ...
        with tab2:
            ...
            if pick:
                ...
                if matching_futures.empty:
                    ...
                    row3[2].metric("Benzer Maç Sayisi", int(stats.get('N', 0)))
                    ...
                    st.markdown("### ?? Benzer Maçlara Göre Dagilim (Top 10)")
                    top10 = sim_df.head(10).copy()
        ...
        with tab4:
            ...
            if log_df.empty:
                ...
                def _build_hit_df(df):
                    ...
                    out.loc[out["PICK_GROUP"] == "BTTS", "HIT"] = (
                        out["PICK_RESULT_NORM"] == out["RES_RESULT_BTTS"]
                    )
                ...
                if stats_enabled:
                    ...
                    if not hit_view.empty:
                        ...
                        # ROI: 1 birim bahis (dogruysa odd-1, yanlissa -1)
                ...
                if not res_ready_view.empty:
                    total_hits = int(res_ready_view["HIT"].sum())
                    total_cnt = int(len(res_ready_view))
                    st.write(f"Gunluk basari: {total_hits}/{total_cnt} ({(total_hits/total_cnt):.1%})")
                    ...
                    league_summary = res_ready_view.groupby("League").agg(
                        count=("HIT", "count"),
                        sum=("HIT", "sum"),
                        roi=("ROI", "mean"),
                    )
                    if not league_summary.empty:
                        league_summary["rate"] = league_summary["sum"] / league_summary["count"]
                        st.dataframe(
                            league_summary.reset_index().style.format({"rate": "{:.1%}", "roi": "{:+.2f}"}),
                            use_container_width=True,
                        )
                    if "POLICY_FLAG" in res_ready_view.columns:
                        policy_summary = res_ready_view.groupby("POLICY_FLAG").agg(
                            count=("HIT", "count"),
                            sum=("HIT", "sum"),
                            roi=("ROI", "mean"),
                        )
                        if not policy_summary.empty:
                            policy_summary["rate"] = policy_summary["sum"] / policy_summary["count"]
                            st.dataframe(
                                policy_summary.reset_index().style.format({"rate": "{:.1%}", "roi": "{:+.2f}"}),
                                use_container_width=True,
                            )
                        pf = res_ready_view[res_ready_view["POLICY_FLAG"] == 1]
                        if not pf.empty:
                            pf_grp = pf.groupby("PICK_GROUP").agg(
                                count=("HIT", "count"),
                                sum=("HIT", "sum"),
                                roi=("ROI", "mean"),
                            )
                            if not pf_grp.empty:
                                pf_grp["rate"] = pf_grp["sum"] / pf_grp["count"]
                                st.dataframe(
                                    pf_grp.reset_index().style.format({"rate": "{:.1%}", "roi": "{:+.2f}"}),
                                    use_container_width=True,
                                )
                            pf_league = pf.groupby("League").agg(
                                count=("HIT", "count"),
                                sum=("HIT", "sum"),
                                roi=("ROI", "mean"),
                            )
                            if not pf_league.empty:
                                pf_league["rate"] = pf_league["sum"] / pf_league["count"]
                                st.dataframe(
                                    pf_league.reset_index().style.format({"rate": "{:.1%}", "roi": "{:+.2f}"}),
                                    use_container_width=True,
                                )

                show_general = True
                if selected_date and not res_ready_view.empty and not res_ready_all.empty:
                    show_general = len(res_ready_view) != len(res_ready_all)

                if show_general and not res_ready_all.empty:
                    total_hits = int(res_ready_all["HIT"].sum())
                    total_cnt = int(len(res_ready_all))
                    st.write(f"Genel basari: {total_hits}/{total_cnt} ({(total_hits/total_cnt):.1%})")
                    ...
                    league_summary = res_ready_all.groupby("League").agg(
                        count=("HIT", "count"),
                        sum=("HIT", "sum"),
                        roi=("ROI", "mean"),
                    )
                    ...
                    if "POLICY_FLAG" in res_ready_all.columns:
                        policy_summary = res_ready_all.groupby("POLICY_FLAG").agg(
                            count=("HIT", "count"),
                            sum=("HIT", "sum"),
                            roi=("ROI", "mean"),
                        )
                        ...
                        pf = res_ready_all[res_ready_all["POLICY_FLAG"] == 1]
                        if not pf.empty:
                            pf_grp = pf.groupby("PICK_GROUP").agg(
                                count=("HIT", "count"),
                                sum=("HIT", "sum"),
